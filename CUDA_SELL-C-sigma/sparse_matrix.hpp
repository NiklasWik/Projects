...
#ifndef DISABLE_CUDA
template <typename Number> // ONE THREAD PER ROW. Not contiguous
__global__ void compute_spmv_thread(const std::size_t n_rows, 
    ...
    if (row < n_rows){
        ...
        for(int i = row_starts[row]; i < row_starts[row + 1]; i++)
            result += values[i] * src[column_indices[i]];
        dst[row] = result;
    }
}
template <typename Number> // ONE WARP PER ROW.  Contiguous
__global__ void compute_spmv_warp(const std::size_t n_rows, 
  ...
  // BlockSize is 32, assuming that a row will not have more than 32 entries.
    ...
    if (row < n_rows){ // should always be true
        ...
        if (tid + row_strt < row_end){
            sum[tid] = values[row_strt + tid] * src[column_indices[row_strt + tid]];
        }
        else {sum[tid] = 0;}
...
#pragma omp parallel for
        for (unsigned int row = 0; row < n_rows + 1; ++row) row_starts[row] = 0;
        ...
        if (memory_space == MemorySpace::CUDA) {
            ...
            AssertCuda(cudaMemcpy(row_starts, host_row_starts,
...
#pragma omp parallel for
    ...
    SparseMatrix(const SparseMatrix &other)
        ...
        if (memory_space == MemorySpace::CUDA) {
            AssertCuda(cudaMalloc(&row_starts, (n_rows + 1) * sizeof(std::size_t)));
            AssertCuda(cudaMemcpy(row_starts, other.row_starts,
                                  (n_rows + 1) * sizeof(std::size_t),
                                  cudaMemcpyDeviceToDevice));

            std::size_t n_entries = 0;
            AssertCuda(cudaMemcpy(&n_entries, other.row_starts + n_rows,
    ...
    std::size_t n_nonzero_entries() const { return n_global_nonzero_entries; }
...
#pragma omp parallel for
    ...
    void set_send_and_receive_information(
        ...
        for (auto i : send_indices) send_size += i.second.size();
        send_data.resize(send_size);
        this->receive_indices = receive_indices;
        std::size_t receive_size = 0;
        for (auto i : receive_indices) receive_size += i.second;
        ...
        if (receive_size > ghost_entries.size()) {
            std::cout
                << "Error, you requested exchange of more entries than what "
    ...
    void apply(const Vector<Number> &src, Vector<Number> &dst) const {
        ...
        for (unsigned int i = 0, count = 0; i < receive_indices.size();
            ...
            MPI_Irecv(receive_data.data() + count,
                      receive_indices[i].second * sizeof(Number), MPI_BYTE,
                      receive_indices[i].first,
                      /* mpi_tag */ 29, communicator, &mpi_requests[i]);
...
#pragma omp parallel for
            ...
            MPI_Isend(send_data.data() + count,
                      send_indices[i].second.size() * sizeof(Number), MPI_BYTE,
                      send_indices[i].first,
                      /* mpi_tag */ 29, communicator,
        ...
        if (memory_space == MemorySpace::CUDA) {
#ifndef DISABLE_CUDA
            // Should probably check if dst/src.memory_space == CUDA
            
            // One thread per row
            /* {
            const unsigned int n_blocks = (n_rows + 32 - 1) / 32;

            compute_spmv_thread<<<n_blocks, 32>>>(
            ...
            // One warp per row 
            {
            const unsigned int n_blocks = n_rows;
            static const unsigned int warp = 32;
            compute_spmv_warp<<<n_blocks, warp>>>(
                n_rows, row_starts, column_indices, values, src.begin(), dst.begin());
...
#pragma omp parallel for
            for (unsigned int row = 0; row < n_rows; ++row) {
                Number sum = 0;
                for (std::size_t idx = row_starts[row];
                     idx < row_starts[row + 1]; ++idx)
...
#ifdef HAVE_MPI
        MPI_Waitall(mpi_requests.size(), mpi_requests.data(),
                    MPI_STATUSES_IGNORE);

        // work on the off-processor data. do not do it in parallel because we
        // do not know whether two parts would work on the same entry of the
        // result vector
    ...
    SparseMatrix copy_to_device() {
        if (memory_space == MemorySpace::CUDA) {
            ...
            exit(EXIT_FAILURE);
            // return dummy
            return SparseMatrix(std::vector<unsigned int>(), MemorySpace::CUDA,
                                communicator);
        } else {
    ...
    std::size_t memory_consumption() const {
        return n_global_nonzero_entries *
                   (sizeof(Number) + sizeof(unsigned int)) +
