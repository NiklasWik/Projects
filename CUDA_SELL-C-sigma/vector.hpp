...
enum class MemorySpace { Host, CUDA };
...
Number mpi_sum(const Number local_sum, MPI_Comm communicator) {
    ...
    if (std::is_same<Number, double>::value) {
        ...
        MPI_Allreduce(&local_sum, &global_sum, 1, MPI_UNSIGNED_LONG, MPI_SUM,
                      communicator);
        return global_sum;
    } else {
...
template <typename Number>
__global__ void set_entries(const std::size_t N, Number scalar,
                            Number *destination) {
    const int idx = threadIdx.x + blockIdx.x * blockDim.x;
    if (idx < N) destination[idx] = scalar;
}

template <typename Number>
__global__ void vector_update(const std::size_t N, Number scalar1,
                              Number scalar2, const Number *source,
                              Number *destination) {
    const int idx = threadIdx.x + blockIdx.x * blockDim.x;
    if (idx < N)
...
__global__ void do_dot(unsigned int n, 
                       Number *vector1, 
                       Number *vector2,
                       Number *result) {
...
class Vector {
   public:
    ...
    Vector(const std::size_t global_size,
           ...
           const MemorySpace memory_space, const MPI_Comm communicator)
    ...
    Vector(const Vector &other)
        ...
        if (memory_space == MemorySpace::CUDA) {
            AssertCuda(cudaMemcpy(data, other.data, local_size * sizeof(Number),
...
#pragma omp parallel for simd
    ...
    Number &operator()(const std::size_t index) { return data[index]; }
...
#pragma omp parallel for simd
            for (std::size_t i = 0; i < local_size; ++i) data[i] = scalar;
        }
    }
    ...
    void sadd(const Number my_scalar, const Number other_scalar,
              const Vector &other) {
        assert_size(other);
        ...
        if (memory_space == MemorySpace::CUDA) {
            ...
            const unsigned int n_blocks =
                (local_size + block_size - 1) / block_size;
            vector_update<<<n_blocks, block_size>>>(local_size, my_scalar,
                                        other_scalar, other.begin(), begin());
...
#pragma omp parallel for simd
    ...
    Number norm_square() const { return dot(*this); }
    ...
    Number dot(const Vector &other) const {
        ...
        if (memory_space == MemorySpace::CUDA) {
            ...
            const unsigned int n_blocks =
                (local_size + block_size - 1) / block_size;
            do_dot<block_size, Number><<<n_blocks, block_size>>>(
                local_size, data, other.data, result_device);
...
#pragma omp parallel for reduction(+ : local_sum)
    ...
    Number *begin() { return data; }

    const Number *begin() const { return data; }
    ...
    Vector copy_to_device() {
        if (memory_space == MemorySpace::CUDA) {
            ...
            AssertCuda(cudaMemcpy(other.begin(), begin(), sizeof(Number)*local_size,
    ...
    std::size_t size() const { return global_size; }

    std::size_t size_on_this_rank() const { return local_size; }
    ...
    void assert_size(const Vector &other) const {
        if (local_size != other.local_size) {
            std::cout << "The local sizes of the two vectors " << local_size
                      << " vs " << other.local_size << " do not match"
                      << std::endl;
            std::abort();
        }
    }
