...
#include "sparse_matrix_scs.hpp"
#include "vector.hpp"

template <typename Number, template <typename> class SpMatrix>
SpMatrix<Number> fill_sparse_matrix(const unsigned int size,
...
#pragma omp parallel for collapse(2)
    ...
    Number entries[27] = {-1. / 12., -1. / 6., -1. / 12., -1. / 6.,  0.,
    ...
    SpMatrix<Number> sparse(row_lengths, memory_space, MPI_COMM_SELF);
        ...
        for (int iz = 0; iz < size; ++iz)
            for (int iy = 0; iy < size; ++iy)
                for (int ix = 0; ix < size; ++ix) {
                    // go through all entries in the current row, clipping away
                    // the parts on the boundary of the cube
                    ...
                    for (int iiz = -1, count = 0; iiz <= 1; ++iiz)
                        for (int iiy = -1; iiy <= 1; ++iiy)
                            for (int iix = -1; iix <= 1; ++iix, ++count)
                                if (iz + iiz >= 0 && iz + iiz < size &&
                                    iy + iiy >= 0 && iy + iiy < size &&
                                    ix + iix >= 0 && ix + iix < size) {
                                    col_indices.push_back(
                                        row + (iiz * size + iiy) * size + iix);
...
void run_test(const long long N, const long long n_repeat) {
    ...
    SpMatrix<Number> matrix = fill_sparse_matrix<Number, SpMatrix>(N, host_space);

    std::cout << "Matrix memory consumption: " 
              << 1e-9 * (matrix.memory_consumption())
              << " GB" << std::endl;

    // Copy matrix to device
    cudaDeviceSynchronize();
    const auto t1_m = std::chrono::steady_clock::now();
    SpMatrix<Number> matrix_dev = matrix.copy_to_device();
    cudaDeviceSynchronize();
    const double time_m =
        std::chrono::duration_cast<std::chrono::duration<double>>(
            std::chrono::steady_clock::now() - t1_m)
            .count();

    std::cout << "Matrix -> device: " << time_m << " seconds or "
              << 1e-9 * (matrix.memory_consumption()) / time_m << " GB/s "
              << std::endl;
...
#pragma omp parallel for collapse(2)
    ...
    const double time_v =
        std::chrono::duration_cast<std::chrono::duration<double>>(
            std::chrono::steady_clock::now() - t1_v)
            .count();

    std::cout << "Vectors -> device: " << time_v << " seconds or "
              << 1e-9 * (2 * dst.size() * sizeof(Number)) / time_v << " GB/s "
              << std::endl;
        ...
        for (unsigned int iz = 0; iz < N; ++iz) {
            ...
            for (unsigned int iy = 0; iy < N; ++iy)
                for (unsigned int ix = 0; ix < N; ++ix) {
                    double local_error =
                        PI * PI * 3 *
                            std::sin(PI * static_cast<double>(ix + 1) /
                                     (N + 1)) *
                            std::sin(PI * static_cast<double>(iy + 1) /
                                     (N + 1)) *
                            std::sin(PI * static_cast<double>(iz + 1) /
                                     (N + 1)) -
                        dst(iz * N * N + iy * N + ix);
...
#pragma omp critical
        ...
        std::cout << "L2 discretization error with sparse matrix: "
...

    {
        ...
        std::cout << "Mat-vec of size " << src.size() << ": " << time / n_repeat
                  << " seconds or "
                  << 1e-9 * n_repeat *
                         (matrix.memory_consumption() +
                          2 * dst.size() * sizeof(Number)) /
                         time
                  << " GB/s " << std::endl;
    }

    {
        ...
        const auto info = solve_with_conjugate_gradient(
            500, 1e-12, matrix_dev, dst_device, result_device);
...
int main(int argc, char **argv) {
    ...
    if (N == -1)
        for (unsigned long long NN = 24; NN < 500; NN += 24) {
            if (matrix == "CRS") {
              if (number == "double")
                  run_test<double, SparseMatrix>(NN, n_repeat);
              else
                  run_test<float, SparseMatrix>(NN, n_repeat);
          } else {
              if (number == "double")
                  run_test<double, SparseMatrix_SCS>(NN, n_repeat);
              else
                  run_test<float, SparseMatrix_SCS>(NN, n_repeat);
          }
        }
    else {
        if (matrix == "CRS") {
            if (number == "double")
                run_test<double, SparseMatrix>(N, n_repeat);
            else
                run_test<float, SparseMatrix>(N, n_repeat);
        } else {
          std::cout << "SCS! \n" << std::endl;
            if (number == "double")
                run_test<double, SparseMatrix_SCS>(N, n_repeat);
            else
                run_test<float, SparseMatrix_SCS>(N, n_repeat);
        }
    }
